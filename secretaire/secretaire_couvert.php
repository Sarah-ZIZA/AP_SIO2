...
<body>
    ...
    <main>
        <form method="get" class="search" action="secretaire_couvert.php" onsubmit="scrollToPage5()">
            ...
            try {
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    if (isset($_POST['supprimer']) && !empty($_POST['selection'])) {
                        ...
                        $requeteSuppression = $connexion->prepare("DELETE FROM  $table5 WHERE patient_id  IN ($placeholders)");
                        $requeteSuppression->execute($ids);
                        header("Location: secretaire_couvert.php"); // Redirection pour éviter la double soumission
                        exit();
                    } elseif (isset($_POST['supprimer_unique'])) {
                        $id = $_POST['supprimer_unique'];
                        $requeteSuppression = $connexion->prepare("DELETE FROM $table5 WHERE patient_id  = ?");
                ...
                echo "<form method='post' action='secretaire_couvert.php'>";
                echo "<table border='2'>";
